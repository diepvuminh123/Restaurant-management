...
import { IoSearchOutline, IoCalendarClearOutline, IoAdd } from 'react-icons/io5';
...
const TakeawayOrdersScreen = () => {
  ...
  const onOpenCreateModal = () => {
    if (cartLoading) {
      toast.info('Đang tải giỏ hàng, vui lòng thử lại sau ít giây.');
      return;
    }
  ...
  const onCreateOrder = async () => {
    ...
    if (cartItems.length === 0) {
      toast.error('Giỏ hàng đang trống. Vui lòng chọn món trước khi tạo đơn.');
      return;
    }
      ...
      {isCreateModalOpen && (
        <div className="create-order-modal-backdrop" role="presentation" onClick={onCloseCreateModal}>
          ...
          >
            ...
            <p className="create-order-modal__subtitle">
              Dùng giỏ hàng hiện tại để tạo đơn. Nếu chưa có món, hãy qua trang menu để chọn trước.
            </p>
            ...
            {cartItems.length > 0 && (
              <div className="create-order-cart-items">
                ...
                {cartItems.length > 4 && <p>...và {cartItems.length - 4} món khác</p>}
              </div>
            )}
